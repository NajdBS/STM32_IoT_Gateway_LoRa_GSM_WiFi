...
int SX1278_LoRaEntryRx2(int x, uint8_t length, uint32_t timeout)
{
	uint8_t addr;
	...
	//Open RxDone interrupt & Timeout
	config_register( LR_RegIrqFlagsMask, 0x3F);
  ...
  //CLEAR ALL FLAG
	...
	//Payload Length 21byte(this register must difine when the data long of one byte in SF is 6)
	config_register(LR_RegPayloadLength,length);
  ...
  readBytes = 0;
	while (1)
	{
	  //Rx-on going RegModemStat
		if ((read_adress(LR_RegModemStat) & 0x04) == 0x04)  
		{		
			return 1;
		}
		timeout--;
